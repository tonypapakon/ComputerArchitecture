...
Μελετώντας το αρχείο starter_se.py καταλήξαμε στα εξής συμπεράσματα για τις βασικές παραμέτρους που έχει περάσει στον gem5 για το σύστημα προς εξομοίωση:
    ...
    CPU:Εφόσον,δώσαμε σαν όρισμα στην εντολή εκτέλεσης της εξομοίωσης να χρησιμοποιηθεί Minor model CPU βλέπουμε από το starter_se.py ότι θα δημιουργηθούν μία L1 instruction cache,μία L1 data cache,μία L2 cache,μία instruction to bus(itb) walker cache και μία data to bus(dtb) walker cache.Επίσης,έχει by default ένα πυρήνα.
    Συχνότητα λειτουργίας:Έχει οριστεί by default από το αρχείο στα 4GHz.
    Caches:Τις caches που θα δημιουργηθούν τις αναφέραμε παραπάνω στο κομμάτι που αφορά την CPU.Επίσης,ορίζεται το μήκος της γραμμής των cache στα 64 bytes.Κάτι άλλο που παρατηρήσαμε είναι ότι αν έχουμε cluster από CPUs αυτές θα έχουνε private L1 caches και μία διαμοιραζόμενη L2 cache.
    Μνήμη:Σχετικά με την μνήμη βλέπουμε ότι το μέγεθος της φυσικής μνήμης ορίζεται by default στα 2GB.Το αρχείο ορίζει και το εύρος της φυσικής μνήμης έτσι ώστε να αποφασιστεί που θα κάνει map τις DRAMS.Ο τύπος της SDRAM που χρησιμοποιεί είναι by default o DDR3_1600_8x8.
    Άλλα στοιχεία:Το αρχείο starter_se.py υλοποιεί ένα δίαυλο και ένα σύστημα μνήμης προς τα έξω από το chip.
